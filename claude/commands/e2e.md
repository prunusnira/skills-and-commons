...
4. HTTP 200~399가 아니면 `apps/service`에서 `pnpm dev:loc`을 기동하고 PID와 로그 경로를 기록한다. 최대 120초 동안 준비를 확인한다.
5. 세션을 만든다.

```bash
SESSION_ID=$(date +%Y%m%d-%H%M%S)
SESSION_DIR="apps/service/e2e/.session/$SESSION_ID"
mkdir -p "$SESSION_DIR"
printf '%s\n' "$SESSION_DIR" > apps/service/e2e/.session/current
```
...
- `READY`: 케이스, mock, oracle, 불확실성 요약을 사용자에게 보여주고 승인받는다.
...
Executor는 계획의 mock과 인증 조건을 먼저 적용한 뒤 각 케이스를 실행한다. 실제 locator, URL, 주요 network request, assertion 결과, 실패 screenshot을 기록한다.
