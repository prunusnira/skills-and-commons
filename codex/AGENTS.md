...
사용자가 claude commands에 정의된 command 이름을 언급하면, Codex는 먼저 `.claude/commands/<command-name>.md`를 찾아 읽고 해당 절차를 따른다. 코드 리뷰 또는 `/ocr` 요청에서는 `review` 스킬을 사용해 1~4단계 중요도와 리포트 형식을 적용한다.
